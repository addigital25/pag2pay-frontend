...
export default function PlatformSettings() {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (settings.images?.favicon) {
            setFaviconPreview(`http://localhost:3001${settings.images.favicon}`)
          }

          if (settings.images?.logo) {
            setLogoPreview(`http://localhost:3001${settings.images.logo}`)
          }

          if (settings.images?.achievementLogo) {
            setAchievementLogoPreview(`http://localhost:3001${settings.images.achievementLogo}`)
          }
  ...
  const handleSaveImages = async () => {
    try {
      ...
      // Enviar para a API
      const response = await fetch('http://localhost:3001/api/platform/settings/images', {
        method: 'POST',
        body: formData,
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('platformToken')}`
  ...
  const handleSaveTexts = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/platform/settings/texts', {
        method: 'POST',
        headers: {
  ...
  const handleSaveExtras = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/platform/settings/extras', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('platformToken')}`
